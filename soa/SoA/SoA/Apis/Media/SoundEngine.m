...
- (void) playAudioFile{
    ...
    NSString* fileName = [[NSBundle mainBundle] pathForResource:@"duration" ofType:@"caf"];
...
- (void)stopSound:(NSString*)soundKey
{
    ...
    if (numVal == nil) return;
    ALuint sourceID = [numVal unsignedIntValue];
...
-(void)cleanUpOpenAL:(id)sender
{
    ...
    for (NSNumber * sourceNumber in [self.soundDictionary allValues]) {
        ALuint sourceID = [sourceNumber unsignedIntValue];
    ...
    for (NSNumber * bufferNumber in bufferStorageArray) {
        ALuint bufferID = [bufferNumber unsignedIntValue];
